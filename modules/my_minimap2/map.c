...
{
	int i, j, k, rep_len, qlen_sum, n_regs0, n_mini_pos;
	...
	if (n_regs0 > 0) {
		...
		for (strand = 0; strand <= 1; strand++) {
			...
			int min_seed_count = regs0[0].cnt / 10;
			int n_target_region;

		        n_target_region = 0;
			for (j = 0; j < n_regs0; ++j) {
				i_start = regs0[j].as, i_end = regs0[j].as + regs0[j].cnt - 1;
				if (a[i_start].x>>63 != strand) continue;

				if (n_target_region >= max_region && regs0[j].cnt < min_seed_count) {
					continue;
				}
			...
			fprintf(opt->output_chain_info_fp, "%s %d %d %d", qname, strand, n_target_region, seq_len);
			n_target_region = 0;
			for (j = 0; j < n_regs0; ++j) {
				i_start = regs0[j].as, i_end = regs0[j].as + regs0[j].cnt - 1;
				if (a[i_start].x>>63 != strand) continue;
				...
				fprintf(opt->output_chain_info_fp, " %d %d %d %d %s %d %d %d", n_target_region, regs0[j].cnt, \
					(int32_t)a[i_start].y, (int32_t)a[i_end].y, mi->seq[a[i_start].x<<1>>33].name, (int32_t)a[i_start].x, (int32_t)a[i_end].x, \
					regs0[j].score);

				n_target_region++;
			}

			fprintf(opt->output_chain_info_fp, "\n");
		}

...
int main(int argc, char *argv[])
{
	...
	const char *opt_str = "2aSDw:k:K:t:r:f:Vv:g:G:I:d:XT:s:x:Hcp:M:n:z:A:B:O:E:m:N:Qu:R:hF:LC:yYP:5:";
	...
	while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >= 0) {
		if (c == 'w') ipt.w = atoi(o.arg);
			...
			if (*s == ',') opt.e2 = strtol(s + 1, &s, 10);
                        ...
                        strcpy(opt.output_chain_info_file, o.arg);
                        opt.output_chain_info_fp = fopen(opt.output_chain_info_file,"w");
                        pthread_mutex_init(&opt.mutex, NULL);
		}
	}
...
//		fprintf(fp_help, "    -v INT       verbose level [%d]\n", mm_verbose);
	...
	if (opt.output_chain_info_fp) {
		fclose(opt.output_chain_info_fp);
		pthread_mutex_destroy(&opt.mutex);
	}
